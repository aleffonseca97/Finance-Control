...
import { parseMonthYearParams, groupByDay } from '@/lib/date-utils'
...
import { formatCurrency, getMonthTitle } from '@/lib/i18n/format'
...
export default async function InvestimentosPage({
  ...
  const t = await getTranslations('dashboard.investments')
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-8">
      ...
      <InvestmentListCard
        ...
        emptyMessage={tShared('noInvestmentsInPeriod')}
                      ...
                      <div className="min-w-0">
                        <p className="font-medium">
                          {item.label
                            ? item.label
                                .split(' → ')
                                .map((part) => localizeStoredLabel(part, locale))
                                .join(' → ')
                            : localizeStoredLabel(item.category.name, locale)}
                        </p>
                        <p className="text-xs text-muted-foreground">
                          {item.label ? tShared('reserveToWallet') : tShared('totalInCategory')}
                        </p>
                      </div>
                    </div>
                    <span className="shrink-0 font-semibold tabular-nums text-blue-500">
                      {formatCurrency(item.total, locale, currency)}
                    </span>
                  </li>
                ))}
              </ul>
