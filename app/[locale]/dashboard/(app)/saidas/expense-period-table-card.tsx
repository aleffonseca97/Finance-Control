...
import { compareLocale, formatCurrency, formatDate } from '@/lib/i18n/format'
...
export function ExpensePeriodTableCard({
  ...
  const generalGroup = tForms('generalGroup')
  ...
  }, [categories, generalGroup, locale])

  const categoriesByGroup = useMemo(() => {
    const map = new Map<string, Category[]>()
    for (const cat of categories) {
      const g = cat.group?.trim() || generalGroup
  ...
  const mainCategories = useMemo(() => {
    ...
    items.forEach((item) => {
      groups.add(item.category.group?.trim() || t('noMainCategory'))
    })

    return Array.from(groups).sort((a, b) => compareLocale(a, b, locale))
  }, [items, locale, t])
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      const group = item.category.group?.trim() || t('noMainCategory')
      const date = formatDate(item.date, locale).toLowerCase()
  ...
  }, [items, mainCategory, paymentFilter, search, locale, t])

  const sortedItems = useMemo(() => {
    const cloned = [...filteredItems]

    cloned.sort((a, b) => {
      const groupA = a.category.group?.trim() || t('noMainCategory')
      const groupB = b.category.group?.trim() || t('noMainCategory')

      let compareValue = 0

      if (sortField === 'category') {
        compareValue = compareLocale(a.category.name, b.category.name, locale)
      } else if (sortField === 'mainCategory') {
        compareValue = compareLocale(groupA, groupB, locale)
    ...
    return cloned
  }, [filteredItems, sortDirection, sortField, locale, t])
  ...
  return (
    <Card className="dashboard-bento-card-muted overflow-hidden shadow-md">
      ...
      <CardContent className="space-y-4 px-4 pb-4 sm:px-6 sm:pb-6">
        <div className="flex flex-col gap-2">
          ...
          <div
            ...
            role="group"
            aria-label={tForms('labels.filterByCard')}
          >
            {(
              [
                { value: 'all' as const, label: t('paymentAll') },
                { value: 'card' as const, label: t('paymentCard') },
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-[minmax(0,1fr)_260px_auto]">
          <input
            ...
            placeholder={tForms('placeholders.filterCategory')}
            ...
            aria-label={tForms('labels.filterTransactions')}
          />
          <select
            value={mainCategory}
            onChange={(event) => setMainCategory(event.target.value)}
            className="h-10 rounded-md border bg-background px-3 text-sm outline-none transition-colors focus:border-ring focus:ring-2 focus:ring-ring/30"
            aria-label={tForms('labels.filterMainCategory')}
          >
            <option value="all">{t('allMainCategories')}</option>
        ...
        {sortedItems.length === 0 ? (
          ...
          <div className="space-y-3">
              ...
              <thead className="bg-muted/60 text-left text-xs uppercase tracking-wide text-muted-foreground">
                <tr>
                  <th className="px-4 py-3 font-medium">
                    ...
                    >
                      {tForms('labels.category')} <span aria-hidden>{sortIndicator('category')}</span>
                    </button>
                  </th>
                  <th className="px-4 py-3 font-medium">
                    <button
                      ...
                      className="inline-flex items-center gap-2 transition-colors hover:text-foreground"
                    >
                      {tForms('labels.mainCategory')} <span aria-hidden>{sortIndicator('mainCategory')}</span>
                    </button>
                  </th>
                  <th className="px-4 py-3 font-medium">
                    <button
                      ...
                      className="inline-flex items-center gap-2 transition-colors hover:text-foreground"
                    >
                      {tForms('labels.date')} <span aria-hidden>{sortIndicator('date')}</span>
                    </button>
                  </th>
                  <th className="px-4 py-3 text-right font-medium">
                    ...
                    >
                      {tForms('labels.amount')} <span aria-hidden>{sortIndicator('amount')}</span>
                    </button>
                  </th>
                </tr>
              </thead>
                    ...
                    >
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span>{localizeStoredLabel(item.category.name, locale)}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-muted-foreground">
                        {localizeStoredLabel(group, locale)}
                      </td>
                      <td className="px-4 py-3 text-muted-foreground">{formatDate(item.date, locale)}</td>
                      <td className="px-4 py-3">
                        <div
            ...
            <div className="flex flex-col gap-2 text-sm text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
              <p>
                {t('showingRecords', { shown: paginatedItems.length, total: sortedItems.length })}
              </p>
              <div className="flex items-center gap-2">
                <button
                  type="button"
      ...
      >
        {detailItem && (
          <>
            <DialogHeader onClose={() => setDetailItem(null)}>{t('expenseDetailTitle')}</DialogHeader>
            <div className="space-y-4 pt-1">
              <div className="flex flex-wrap gap-x-6 gap-y-1 text-sm text-muted-foreground">
                <span>
                  <span className="font-medium text-foreground">{tForms('labels.date')}:</span>{' '}
                  {formatDate(detailItem.date, locale)}
                </span>
                <span className="font-semibold text-red-500 tabular-nums">
                  {formatCurrency(detailItem.amount, locale, currency)}
                </span>
              </div>
              <div className="space-y-2">
                <p className="text-sm font-medium text-foreground">{tForms('labels.description')}</p>
              ...
              <div className="space-y-2">
                <Label htmlFor="expense-detail-category">{tForms('labels.category')}</Label>
                <Select
              ...
              <div className="flex flex-wrap justify-end gap-2 pt-2">
                ...
                >
                  {saving ? tForms('buttons.saving') : t('saveCategory')}
                </Button>
              </div>
            </div>
          </>
